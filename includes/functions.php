...
function student_id(): ?int {
    ...
    if (!empty($currentUser['mahasiswa_id'])) {
        return (int) $currentUser['mahasiswa_id'];
    }

    $q = db()->prepare('SELECT id FROM mahasiswa WHERE user_id = ? LIMIT 1');
...
function lecturer_id(): ?int {
    ...
    if (!empty($currentUser['dosen_id'])) {
        return (int) $currentUser['dosen_id'];
    }

    $q = db()->prepare('SELECT id FROM dosen WHERE user_id = ? LIMIT 1');
    $q->execute([(int) $currentUser['id']]);
    $id = $q->fetchColumn();
